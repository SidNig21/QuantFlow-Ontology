...
import { buildKernelDriftInstallPlan } from "../kernel-drift.ts";
...
function assertInstallPlan(): string | null {
  ...
  const expectedNames = ["qf-kernel", "kernel-drift"] as const;
  const expectedCwds = [
    join(REPO_ROOT, "packages/qf-kernel"),
    join(REPO_ROOT, "qa/gates/kernel-drift"),
  ] as const;

  for (const name of expectedNames) {
    if (!built.entries.some((entry) => entry.name === name)) {
      return `kernel-drift: missing install plan entry: ${name}`;
    }
  }
  if (built.entries.length !== expectedNames.length) {
    return `kernel-drift: install plan must contain exactly ${expectedNames.join(" then ")}`;
  }
  for (let i = 0; i < expectedNames.length; i += 1) {
    if (built.entries[i]!.name !== expectedNames[i]) {
      return `kernel-drift: install plan order must be ${expectedNames.join(" then ")}`;
    }
    if (built.entries[i]!.cwd !== expectedCwds[i]) {
      return `kernel-drift: install plan ${expectedNames[i]} cwd must be ${expectedCwds[i]}`;
    }
  }
  return null;
}
...
async function main(): Promise<number> {
  ...
  if (planError) {
    console.error(`kernel-drift FAIL: ${planError}`);
    return 1;
  }

...
async function runLiveR17C14Proof(falsifier: Falsifier | null = null): Promise<boolean> {
  ...
  try {
    ...
    assert(world.objects.length === 16 && world.links.length === 20, "C14/model-complete production projection is not exact 16/20");
    ...
    if (falsifiesC14(falsifier, "back-to-world")) await c14SimpleMutation(live, "back-to-world", `const node=document.querySelector('[data-qf-back-to-world]');if(!(node instanceof HTMLElement)||!node.parentNode)throw new Error('Back to world missing');const parent=node.parentNode,next=node.nextSibling,clone=node.cloneNode(true);parent.replaceChild(clone,node);clone.click();return()=>{if(clone.parentNode===parent)parent.replaceChild(node,clone);else parent.insertBefore(node,next?.parentNode===parent?next:null);};`, "document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='DEFAULT'");
    ...
    passC14SimpleReceipt("back-to-world", backReceipt);

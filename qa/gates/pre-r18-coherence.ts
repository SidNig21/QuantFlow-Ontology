...
function independentObjectFields(path: string, type: string, id: string): Json {
  ...
  if (type === "evaluation") { fields.semantic_markers = ["EVALUATION"]; fields.critic_session_id = outgoing.find((link) => link.kind === "performed_by")?.to_id ?? null; fields.target_artifact_id = dbRows(path, "SELECT links.from_id FROM links JOIN artifact ON artifact.id = links.from_id WHERE links.kind = 'evaluated_by' AND links.to_id = ?", id)[0]?.from_id ?? null; fields.report_artifact_id = row.publication_report_id ?? null; }
  if (type === "agent_session") { const definitionId = outgoing.find((link) => link.kind === "spawned_from")?.to_id; fields.definition_id = definitionId ?? null; const definition = definitionId ? dbRows(path, "SELECT role, display_name, runtime_profile, capability_groups FROM agent_definition WHERE id = ?", definitionId)[0] : undefined; if (definition) { fields.role = definition.role; fields.display_name = definition.display_name; fields.runtime_profile = definition.runtime_profile; fields.capability_groups = typeof definition.capability_groups === "string" ? JSON.parse(definition.capability_groups) : definition.capability_groups; } }
  if (type === "artifact") { const producer = dbRows(path, "SELECT from_id FROM links WHERE kind = 'produces' AND to_id = ?", id)[0]; fields.producer_id = producer?.from_id ?? null; fields.producer_type = producer ? (dbRows(path, "SELECT id FROM run WHERE id = ?", producer.from_id).length > 0 ? "run" : dbRows(path, "SELECT id FROM agent_session WHERE id = ?", producer.from_id).length > 0 ? "agent_session" : null) : null; if (producer && dbRows(path, "SELECT id FROM run WHERE id = ?", producer.from_id).length > 0) fields.run_id = producer.from_id; const source = dbRows(path, "SELECT source_work FROM evaluation").map((entry) => typeof entry.source_work === "string" ? JSON.parse(entry.source_work) as Json : entry.source_work as Json).find((entry) => entry?.result_artifact_id === id); fields.source_run_id = source?.run_id ?? null; fields.source_task_id = source?.source_task_id ?? null; if (row.kind !== "report" && row.kind !== "evaluation_findings") fields.semantic_markers = ["RAW ARTIFACT"]; if (row.kind === "report") { fields.semantic_markers = ["PUBLISHED REPORT", "CURRENT AUTHORITY"]; fields.gating_evaluation_id = dbRows(path, "SELECT from_id FROM links WHERE kind = 'gates' AND to_id = ?", id)[0]?.from_id ?? null; fields.current_authority = true; fields.historical = false; } }
  return fields;
}
...
function independentOracleFields(path: string, ids: Record<string, string>): Record<string, unknown> {
  ...
  for (const field of ["title", "description", "status"]) bind(`\${field:review_task:${field}}`, "task", ids.review_task_id, field);
  for (const field of ["status", "label", "definition_id", "role", "display_name", "runtime_profile", "capability_groups"]) {
    bind(`\${field:director_session:${field}}`, "agent_session", ids.director_session_id, field);
    bind(`\${field:executor_session:${field}}`, "agent_session", ids.executor_session_id, field);
    bind(`\${field:critic_session:${field}}`, "agent_session", ids.critic_session_id, field);
  }
  ...
  bind("${field:hypothesis:status}", "hypothesis", "hypothesis-r17-gate", "status");
  for (const field of ["content_hash", "receipt", "semantic_markers", "producer_id", "producer_type", "source_run_id", "source_task_id"]) bind(`\${field:result_artifact:${field}}`, "artifact", ids.result_artifact_id, field);
  for (const field of ["content_hash", "receipt", "semantic_markers", "producer_id", "producer_type", "source_run_id", "source_task_id"]) bind(`\${field:worker_evidence_artifact:${field}}`, "artifact", ids.worker_evidence_artifact_id, field);
  for (const field of ["content_hash", "receipt", "semantic_markers", "producer_id", "producer_type", "source_run_id", "source_task_id"]) bind(`\${field:grade_artifact:${field}}`, "artifact", ids.grade_artifact_id, field);
  for (const field of ["content_hash", "receipt", "producer_id", "producer_type", "source_run_id", "source_task_id"]) bind(`\${field:findings_artifact:${field}}`, "artifact", ids.findings_artifact_id, field);
  for (const field of ["content_hash", "receipt", "semantic_markers", "producer_id", "producer_type", "source_run_id", "source_task_id", "gating_evaluation_id", "current_authority", "historical"]) bind(`\${field:report_artifact:${field}}`, "artifact", ids.report_artifact_id, field);
  for (const field of ["critic_findings_ref", "findings_artifact_id", "metrics", "overall", "publication_report_id", "review_task_id", "rubric", "run_metrics", "source_work", "rationale", "broker_invocation_id", "semantic_markers", "target_artifact_id", "report_artifact_id"]) bind(`\${field:evaluation:${field}}`, "evaluation", ids.evaluation_id, field);
  return facts;
}
...
function resolveR17Bindings(path: string, oracle: Json, directorSessionId: string, strategyId: string, continuation: { reviewTaskId: string; criticSessionId: string; evaluationId: string; findingsArtifactId: string; reportArtifactId: string }): { expected: Json; ids: Record<string, string> } {
  ...
  const workerEvidenceArtifactId = String(dbRows(path, "SELECT json_extract(source_work, '$.result_artifact_id') AS id FROM evaluation WHERE id = ?", continuation.evaluationId)[0]?.id ?? "");
  ...
  const evaluationId = continuation.evaluationId;
  const reviewTaskId = continuation.reviewTaskId;
  const findingsArtifactId = continuation.findingsArtifactId;
  const reportArtifactId = continuation.reportArtifactId;
  ...
  const ids = { dataset_id: datasetId, director_session_id: directorSessionId, executor_session_id: executorSessionId, critic_session_id: continuation.criticSessionId, strategy_id: strategyId, result_artifact_id: resultArtifactId, worker_evidence_artifact_id: workerEvidenceArtifactId, grade_artifact_id: gradeArtifactId, review_task_id: reviewTaskId, evaluation_id: evaluationId, findings_artifact_id: findingsArtifactId, report_artifact_id: reportArtifactId };
  for (const [name, value] of Object.entries(ids)) assert(value.length > 0, `R17 binding ${name} is missing`);
  const replacements: Record<string, unknown> = { "${dataset_id}": ids.dataset_id, "${director_session_id}": ids.director_session_id, "${executor_session_id}": ids.executor_session_id, "${critic_session_id}": ids.critic_session_id, "${strategy_id}": ids.strategy_id, "${result_artifact_id}": ids.result_artifact_id, "${worker_evidence_artifact_id}": ids.worker_evidence_artifact_id, "${grade_artifact_id}": ids.grade_artifact_id, "${review_task_id}": ids.review_task_id, "${evaluation_id}": ids.evaluation_id, "${findings_artifact_id}": ids.findings_artifact_id, "${report_artifact_id}": ids.report_artifact_id, ...independentOracleFields(path, ids) };
...
function consumerProjectionExpectation(world: R17World): { primaryIds: Set<string>; primaryLinks: Set<string>; historyIds: Set<string>; currentMissionIds: Set<string>; stages: string[][] } {
  ...
  const run = objects.filter((object) => object.type === "run").sort((a, b) => `${a.type}:${a.id}`.localeCompare(`${b.type}:${b.id}`)).find((object) => object.fields?.executor_session_id === assigned?.to_id) ?? objects.filter((object) => object.type === "run").sort((a, b) => `${a.type}:${a.id}`.localeCompare(`${b.type}:${b.id}`))[0];
  const raw = byId.get(String(run?.fields?.result_artifact_id ?? "")) ?? byId.get(first(links, (link) => link.kind === "produces" && link.from_id === run?.id && byId.get(link.to_id)?.fields?.kind !== "report" && byId.get(link.to_id)?.fields?.kind !== "evaluation_findings")?.to_id ?? "");
  ...
  const evaluation = byId.get(String(evaluated?.to_id ?? "")) ?? objects.filter((object) => object.type === "evaluation" && (object.fields?.target_artifact_id === raw?.id || (object.fields?.source_work as Json | undefined)?.source_task_id === task?.id)).sort((a, b) => `${a.type}:${a.id}`.localeCompare(`${b.type}:${b.id}`))[0];
  add(evaluation?.id);
...
async function toggleObjectInspect(live: Live, type: string, id: string, expected: "open" | "closed" = "open"): Promise<void> {
  ...
  assert(state.type === type && state.id === id && (open ? state.projection === "CURRENT_MISSION" && state.subject === id && state.dockVisible && state.selected : state.projection !== "LOCAL" && !state.dockVisible && !state.selected), `Dock Inspect did not ${open ? "open" : "close"} the requested ${type}:${id}: ${JSON.stringify(state)}`);
...
async function prepareProjectionObjectCapture(live: Live, type: string, id: string): Promise<void> {
  ...
  assert(proof.state === "CURRENT_MISSION" && proof.subject === id && proof.dockVisible === true && proof.canvasDetails === 0 && String(proof.identity).includes(id) && proof.selected === "true" && proof.selectedCount === 1 && proof.back === true, `projection object Inspect did not enter readable Dock LOCAL for ${type}:${id}: ${JSON.stringify(proof)}`);
...
async function revealMission(endpoint: string): Promise<void> { await waitFor("R17 Mission Open workspace dispatch", async () => await evaluate<boolean | null>(endpoint, "(() => { const tile=[...document.querySelectorAll('.canvas-tile[data-qf-world-type=\"mission\"]')].find((node)=>node.dataset.qfWorldId==='mission-r17-gate');const button=tile?.querySelector('.qf-world-reveal');if(!(button instanceof HTMLElement)||button.hidden||getComputedStyle(button).display==='none'||button.textContent?.trim()!=='Open workspace')return null;button.dispatchEvent(new PointerEvent('click',{bubbles:true,button:0,pointerType:'mouse',isPrimary:true}));return true;})()"), Date.now() + 5_000); }
async function openColdReopenedMission(live: Live, expectedSavedHash: string, expectedIds: string[], expectedLinks: string[]): Promise<void> {
  const durable = await waitFor("cold reopen durable saved identities", async () => await evaluate<Json | null>(live.endpoint, `(async () => { const all=[...document.querySelectorAll('.canvas-tile[data-tile-type][data-tile-id]')];const saved=all.filter((node)=>node.dataset.tileType==='research');const sessions=all.filter((node)=>Boolean(node.dataset.sessionId));if(saved.length!==14||sessions.length!==3)return null;const bytes=new TextEncoder().encode(saved.map((node)=>[node.dataset.tileType??'',node.dataset.tileId??''].join('\\u0000')).sort().join('\\n'));const hash=await crypto.subtle.digest('SHA-256',bytes);const sha=[...new Uint8Array(hash)].map((value)=>value.toString(16).padStart(2,'0')).join('');return sha===${JSON.stringify(expectedSavedHash)}?{saved:saved.length,sessions:sessions.length,sha}:null;})()`), Date.now() + 20_000);
  await assertOrdinaryCanvas(live);
  await waitFor("cold reopen HISTORY tab dispatch", async () => await evaluate<boolean | null>(live.endpoint, "(() => { const tab=document.querySelector('[data-dock-mode=\"HISTORY\"]');if(!(tab instanceof HTMLElement)||tab.hidden||getComputedStyle(tab).display==='none')return null;tab.dispatchEvent(new PointerEvent('click',{bubbles:true,button:0,pointerType:'mouse',isPrimary:true}));return true;})()"), Date.now() + 5_000);
  await waitFor("cold reopen HISTORY Mission reveal dispatch", async () => await evaluate<boolean | null>(live.endpoint, "(() => { const tab=document.querySelector('[data-dock-mode=\"HISTORY\"]');const pane=document.querySelector('[data-dock-primary=\"HISTORY\"]');if(tab?.getAttribute('aria-selected')!=='true'||!(pane instanceof HTMLElement)||pane.hidden||getComputedStyle(pane).display==='none'||pane.getBoundingClientRect().width<=0||pane.getBoundingClientRect().height<=0)return null;const rows=[...pane.querySelectorAll('.kl-row[data-event-id=\"mission-r17-gate\"]')];const reveals=rows.flatMap((row)=>[...row.querySelectorAll('.kl-reveal')]).filter((node)=>node instanceof HTMLElement&&!node.hidden&&getComputedStyle(node).display!=='none'&&node.textContent?.trim()==='Show research world');if(rows.length!==1||reveals.length!==1)return null;reveals[0].dispatchEvent(new PointerEvent('click',{bubbles:true,button:0,pointerType:'mouse',isPrimary:true}));return true;})()"), Date.now() + 5_000);
  await waitFor("CURRENT_MISSION after cold reopen", async () => await evaluate<boolean>(live.endpoint, "document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='CURRENT_MISSION'"), Date.now() + 5_000);
  const proof = await evaluate<{ visible: string[]; links: string[] }>(live.endpoint, "(() => ({visible:[...document.querySelectorAll('.canvas-tile[data-qf-world-type]')].filter((node)=>!node.hidden&&getComputedStyle(node).opacity>0).map((node)=>node.dataset.qfWorldId).sort(),links:[...document.querySelectorAll('.cable-path[data-qf-world-cable-kind]')].map((node)=>[node.dataset.qfWorldCableKind,node.dataset.qfWorldCableFrom,node.dataset.qfWorldCableTo].join('\\u0000')).sort()}))()");
  assert(canonicalJson(proof.visible)===canonicalJson(expectedIds)&&canonicalJson(proof.links)===canonicalJson(expectedLinks), `cold reopen CURRENT_MISSION membership differs: ${JSON.stringify(proof)}`);
  console.log(`pre-r18-coherence: cold-reopen-navigation=PASS ${JSON.stringify(durable)}`);
}
async function assertReopenedParticipantInvariant(live: Live, kernelDb: string, directorSessionId: string, executorSessionId: string, criticSessionId: string, reviewTaskId: string): Promise<void> {
  const clickMode = async (mode: "ACTIVE" | "HISTORY") => { await evaluate<boolean>(live.endpoint, `(() => { const tab=document.querySelector(${JSON.stringify(`[data-dock-mode="${mode}"]`)});if(!(tab instanceof HTMLElement))throw new Error('${mode} tab missing');tab.dispatchEvent(new PointerEvent('click',{bubbles:true,button:0,pointerType:'mouse',isPrimary:true}));return true;})()`); await waitFor(`reopened ${mode} mode`, async () => await evaluate<boolean>(live.endpoint, `document.querySelector(${JSON.stringify(`[data-dock-mode="${mode}"][aria-selected="true"]`)}) !== null`), Date.now() + 5_000); };
  await clickMode("ACTIVE");
  const active = await evaluate<string[]>(live.endpoint, "[...document.querySelectorAll('#dock-sessions-list .srow[data-session-id]')].map((node)=>node.dataset.sessionId).sort()");
  assert(active.length===0, `reopened ACTIVE participant set is not empty: ${JSON.stringify(active)}`); await captureState(live, "10-dock-active-sessions");
  await clickMode("HISTORY");
  const expected = [directorSessionId, executorSessionId, criticSessionId].sort();
  const history = await evaluate<Array<{id:string;runtime:string;work:string;recovery:string}>>(live.endpoint, "[...document.querySelectorAll('#dock-history-list .srow[data-session-id]')].map((node)=>({id:node.dataset.sessionId??'',runtime:node.dataset.qfParticipantRuntime??'',work:node.dataset.qfParticipantWork??'',recovery:node.dataset.qfParticipantRecovery??''})).sort((a,b)=>a.id.localeCompare(b.id))");
  assert(canonicalJson(history.map((row)=>row.id))===canonicalJson(expected)&&history.every((row)=>row.runtime==='stopped'&&row.recovery==='restartable')&&history.find((row)=>row.id===directorSessionId)?.work==='unassigned'&&history.find((row)=>row.id===executorSessionId)?.work==='completed'&&history.find((row)=>row.id===criticSessionId)?.work==='completed', `reopened HISTORY participant invariant differs: ${JSON.stringify(history)}`);
  await evaluate<boolean>(live.endpoint, `(() => { const row=document.querySelector(${JSON.stringify(`#dock-history-list .srow[data-session-id="${executorSessionId}"]`)});if(!(row instanceof HTMLElement))throw new Error('exact executor HISTORY row missing');row.dispatchEvent(new PointerEvent('click',{bubbles:true,button:0,pointerType:'mouse',isPrimary:true}));return true;})()`);
  await waitFor("reopened executor INSPECT", async () => await evaluate<boolean>(live.endpoint, `document.querySelector('[data-dock-primary="INSPECT"]')?.hidden===false&&document.querySelector('#dock-inspect-pane .dock-inspect-id')?.textContent===${JSON.stringify(executorSessionId)}`), Date.now() + 5_000);
  const parity = await evaluate<Json>(live.endpoint, `(() => { const canvas=document.querySelector(${JSON.stringify(`.canvas-tile[data-qf-world-type="agent_session"][data-qf-world-id="${executorSessionId}"]`)});const row=document.querySelector(${JSON.stringify(`#dock-history-list .srow[data-session-id="${executorSessionId}"]`)});const fields=Object.fromEntries([...document.querySelectorAll('#dock-inspect-pane .qf-world-field')].map((node)=>[node.querySelector('.qf-world-field-label')?.textContent??'',node.querySelector('.qf-world-field-value')?.textContent??'']));const axes=(node)=>({runtime:node?.dataset.qfParticipantRuntime??'',work:node?.dataset.qfParticipantWork??'',recovery:node?.dataset.qfParticipantRecovery??''});return{canvas:axes(canvas),history:axes(row),fields};})()`);
  const kernel = dbRows(kernelDb, "SELECT id, status FROM agent_session WHERE id IN (?, ?, ?) ORDER BY id", directorSessionId, executorSessionId, criticSessionId);
  const tasks = dbRows(kernelDb, "SELECT id, status FROM task WHERE id IN (?, ?) ORDER BY id", "task-r17-gate", reviewTaskId);
  const assignments = dbRows(kernelDb, "SELECT from_id, to_id FROM links WHERE kind = 'assigned_to' AND from_id IN (?, ?) ORDER BY from_id, to_id", "task-r17-gate", reviewTaskId);
  const directorAssignments = dbRows(kernelDb, "SELECT from_id FROM links WHERE kind = 'assigned_to' AND to_id = ?", directorSessionId);
  assert(kernel.length===3&&kernel.every((row)=>row.status==='closed')&&tasks.length===2&&tasks.every((row)=>row.status==='done')&&assignments.length===2&&assignments.some((row)=>row.from_id==='task-r17-gate'&&row.to_id===executorSessionId)&&assignments.some((row)=>row.from_id===reviewTaskId&&row.to_id===criticSessionId)&&directorAssignments.length===0, `reopened Kernel/task participant invariant differs: ${JSON.stringify({kernel,tasks,assignments,directorAssignments})}`);
  assert(canonicalJson(parity.canvas)===canonicalJson(parity.history)&&parity.canvas?.runtime==='stopped'&&parity.canvas?.work==='completed'&&parity.canvas?.recovery==='restartable'&&parity.fields?.["runtime state"]==='stopped'&&parity.fields?.work==='completed'&&parity.fields?.recovery==='restartable', `reopened Canvas/HISTORY/Inspect participant parity differs: ${JSON.stringify(parity)}`);
  await captureState(live, "11-selected-participant"); console.log(`pre-r18-coherence: reopened-participant-invariant=PASS executor=${executorSessionId}`);
}
async function assertOrdinaryCanvas(live: Live): Promise<void> { const proof = await waitFor("ORDINARY_CANVAS", async () => await evaluate<Json>(live.endpoint, "(() => { const controls=document.querySelector('#research-world-projection'); const viewer=document.querySelector('#panel-viewer'); const visible=[...document.querySelectorAll('.canvas-tile')].filter((node)=>!node.hidden&&getComputedStyle(node).display!=='none'&&node.getBoundingClientRect().width>0&&node.getBoundingClientRect().height>0); return controls?.dataset.qfProjectionState==='ORDINARY_CANVAS'&&controls.hidden&&viewer?.hasAttribute('data-qf-research-projection-active')===false&&document.querySelectorAll('.cable-path[data-qf-world-cable-kind]').length===0&&!document.querySelector('[data-qf-selected=\"true\"],.cable-path--selected')&&visible.length>0&&visible.every((node)=>getComputedStyle(node).pointerEvents!=='none')?{state:'ORDINARY_CANVAS',visible:visible.length}:null; })()"), Date.now() + 5_000); assert(proof.state === "ORDINARY_CANVAS", `ordinary Canvas proof failed: ${JSON.stringify(proof)}`); }
async function openCurrentMission(live: Live, expectedIds: string[], expectedLinks: string[]): Promise<{ visible: string[]; links: string[]; fullVisible: boolean; selected: boolean; inspectorHidden: boolean }> { await revealMission(live.endpoint); await waitFor("CURRENT_MISSION", async () => await evaluate<boolean>(live.endpoint, "document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='CURRENT_MISSION'"), Date.now() + 5_000); const proof = await evaluate<{ visible: string[]; links: string[]; fullVisible: boolean; selected: boolean; inspectorHidden: boolean }>(live.endpoint, "(() => { const visible=[...document.querySelectorAll('.canvas-tile[data-qf-world-type]')].filter((node)=>!node.hidden&&getComputedStyle(node).opacity>0).map((node)=>node.dataset.qfWorldId).sort(); const links=[...document.querySelectorAll('.cable-path[data-qf-world-cable-kind]')].map((node)=>[node.dataset.qfWorldCableKind,node.dataset.qfWorldCableFrom,node.dataset.qfWorldCableTo].join('\\u0000')).sort(); const full=document.querySelector('[data-qf-world-full]'); const inspector=document.querySelector('#cable-inspector'); return {visible,links,fullVisible:full instanceof HTMLElement&&!full.hidden,selected:Boolean(document.querySelector('.cable-path--selected')),inspectorHidden:!(inspector instanceof HTMLElement)||inspector.hidden}; })()"); assert(canonicalJson(proof.visible) === canonicalJson(expectedIds) && canonicalJson(proof.links) === canonicalJson(expectedLinks), `CURRENT_MISSION membership differs: ${JSON.stringify({ actual: proof, expectedIds, expectedLinks })}`); return proof; }
...
async function assertSettledResultInspect(live: Live, resultArtifactId: string): Promise<void> { const proof = await evaluate<Json>(live.endpoint, `(async () => { const tile=[...document.querySelectorAll('.canvas-tile[data-qf-world-type="artifact"]')].find((node)=>node.dataset.qfWorldId===${JSON.stringify(resultArtifactId)}); if (!(tile instanceof HTMLElement)) throw new Error('exact settled Run-result Artifact tile missing'); tile.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,button:0})); await new Promise((resolve)=>setTimeout(resolve,100)); const pane=document.querySelector('#dock-inspect-pane'); const state=pane?.querySelector('.qf-outcome-state')?.textContent; const receipt=pane?.querySelector('.qf-outcome-receipt')?.textContent; const recordButton=Boolean(pane?.querySelector('.qf-outcome-row [aria-label^="Record settled outcome"]')); return { subject:pane?.dataset.qfProjectionSubject,state,receipt,recordButton }; })()`); assert(proof.subject === resultArtifactId && proof.state === "WIN" && String(proof.receipt).includes("calibration=0.040000") && String(proof.receipt).includes("clv=0.100000") && proof.recordButton === false, `settled Dock Inspect was not bound to exact Run-result Artifact: ${JSON.stringify(proof)}`); await evaluate<boolean>(live.endpoint, "document.querySelector('[data-qf-back-to-world]')?.click(); true"); await assertOrdinaryCanvas(live); }
...
function defaultReceiptExpression(specs: TileReceiptSpec[], expectedIds: string[], expectedLinks: string[], stages: string[][]): string {
  return `(() => {
    ...
    const visibleTiles=expectedIds.map(tile); const visibleRects=visibleTiles.map(rect); const stageIds=[...new Set(stages.flat())]; const stageRects=stageIds.map((id)=>rect(tile(id))); const canvas=document.querySelector('#panel-viewer'); const canvasRect=rect(canvas);
    ...
    const membershipOk=stageIds.length===13&&stages.every((members,index)=>{const label=labels.find((node)=>node.dataset.qfStage===String(index));const memberTiles=members.map(tile);if(!label||label.textContent.trim()!==stageNames[index]||label.closest('.canvas-tile')?.dataset.qfWorldId!==members[0]||memberTiles.some((node)=>!painted(node)))return false;const xs=memberTiles.map((node)=>(rect(node).left+rect(node).right)/2);return Math.max(...xs)-Math.min(...xs)<=2;})&&labels.length===5&&visibleTiles.every(painted)&&[...document.querySelectorAll('.canvas-tile[data-qf-world-type]')].filter(painted).length===expectedIds.length;
    ...
    const union={left:Math.min(...stageRects.map((r)=>r.left)),top:Math.min(...stageRects.map((r)=>r.top)),right:Math.max(...stageRects.map((r)=>r.right)),bottom:Math.max(...stageRects.map((r)=>r.bottom))}; const widthRatio=(union.right-union.left)/canvas.clientWidth;const heightRatio=(union.bottom-union.top)/canvas.clientHeight;
    const boundsOk=widthRatio>=.70&&widthRatio<=.85&&heightRatio>=.45&&heightRatio<=.70&&Math.min(...stageRects.map((r)=>r.width))>=136;
...
function dockReceiptExpression(expectedInventory: Array<{ id: string; name: string }>): string {
  return `(async () => {
    ...
    const painted=(node)=>{if(!(node instanceof Element))return false;const r=rect(node),s=getComputedStyle(node);return !node.hidden&&s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity)>0&&r.width>0&&r.height>0;};
    const inside=(inner,outer)=>inner.left>=outer.left-1&&inner.top>=outer.top-1&&inner.right<=outer.right+1&&inner.bottom<=outer.bottom+1;
    const expected=${JSON.stringify(expectedInventory)};const catalogRows=()=>[...document.querySelectorAll('#dock-species-list .lrow[data-definition-id]')].map((row)=>({id:row.dataset.definitionId??'',name:row.querySelector('b')?.textContent.trim()??''}));const dataRows=()=>[...document.querySelectorAll('#dock-species-list .dock-data-row')].map((row)=>({id:row.dataset.capabilityId??'',capability_class:row.dataset.capabilityClass??'',implementation_version:row.dataset.implementationVersion??'',name:row.querySelector('b')?.textContent.trim()??''}));
...
function historyReceiptExpression(expectedSessionIds: string[], expectedHistoricalReportIds: string[], currentReportId: string, activeMissionId: string): string {
  return `(() => { const expectedSessions=${JSON.stringify(expectedSessionIds)}.slice().sort();const expectedHistoricalReports=${JSON.stringify(expectedHistoricalReportIds)}.slice().sort();const current=${JSON.stringify(currentReportId)};const mission=${JSON.stringify(activeMissionId)};const ledger=[...document.querySelectorAll('#kernel-ledger-list .kl-row')].map((node)=>node.dataset.eventId).filter(Boolean).sort();const sessions=[...document.querySelectorAll('#dock-history-list .srow[data-session-id]')].map((node)=>node.dataset.sessionId).filter(Boolean).sort();const artifacts=[...document.querySelectorAll('.canvas-tile[data-qf-world-type="artifact"]')];const historicalReports=artifacts.filter((node)=>String(node.dataset.qfWorldMarkers??'').split('|').includes('HISTORICAL')).map((node)=>node.dataset.qfWorldId).filter(Boolean).sort();const report=artifacts.find((node)=>node.dataset.qfWorldId===current);const reportStatus=report?.querySelector('.qf-world-status')?.textContent.trim()??'';const reportMarkers=String(report?.dataset.qfWorldMarkers??'');const missionCurrent=ledger.includes(mission)&&ledger.every((id)=>id===mission);return {ok:JSON.stringify(sessions)===JSON.stringify(expectedSessions)&&JSON.stringify(historicalReports)===JSON.stringify(expectedHistoricalReports)&&missionCurrent&&!sessions.includes(current)&&!historicalReports.includes(current)&&reportStatus==='PUBLISHED CURRENT'&&!reportMarkers.split('|').includes('HISTORICAL'),ledger,sessions,expectedSessions,historicalReports,expectedHistoricalReports,reportStatus,reportMarkers,missionCurrent};})()`;
}

async function runHistoryMutationMatrix(live: Live, receipt: string, currentReportId: string): Promise<void> {
  const heterogeneous = await evaluate<boolean>(live.endpoint, `(() => { const ledger=[...document.querySelectorAll('#kernel-ledger-list .kl-row')].map((node)=>node.dataset.eventId);const sessions=[...document.querySelectorAll('#dock-history-list .srow[data-session-id]')].map((node)=>node.dataset.sessionId);return new Set([...ledger,...sessions]).size===3;})()`);
  assert(heterogeneous === false, "C14/history-authority heterogeneous union stayed green"); console.error("pre-r18-coherence: C14/history-authority/heterogeneous-union=RED");
  await c14SimpleMutation(live, "history-authority", `const node=[...document.querySelectorAll('.canvas-tile[data-qf-world-type="artifact"]')].find((entry)=>entry.dataset.qfWorldId===${JSON.stringify(currentReportId)});if(!(node instanceof HTMLElement))throw new Error('current report missing');const value=node.dataset.qfWorldMarkers;node.dataset.qfWorldMarkers=(value?value+'|':'')+'HISTORICAL';return()=>{node.dataset.qfWorldMarkers=value;};`, receipt);
  const addHistoryRow = (id: string) => `const parent=document.querySelector('#dock-history-list');if(!(parent instanceof HTMLElement))throw new Error('Dock HISTORY missing');const node=document.createElement('div');node.className='srow';node.dataset.sessionId=${JSON.stringify(id)};parent.append(node);return()=>node.remove();`;
  await c14SimpleMutation(live, "history-authority", addHistoryRow("active-participant-session"), receipt);
  await c14SimpleMutation(live, "history-authority", `const node=document.querySelector('#dock-history-list .srow[data-session-id]');if(!(node instanceof HTMLElement)||!node.parentNode)throw new Error('terminal HISTORY row missing');const parent=node.parentNode,next=node.nextSibling;node.remove();return()=>parent.insertBefore(node,next?.parentNode===parent?next:null);`, receipt);
  const addHistoricalArtifact = (id: string) => `const parent=document.querySelector('#panel-viewer');if(!(parent instanceof HTMLElement))throw new Error('Canvas missing');const node=document.createElement('div');node.className='canvas-tile';node.dataset.qfWorldType='artifact';node.dataset.qfWorldId=${JSON.stringify(id)};node.dataset.qfWorldMarkers='HISTORICAL';parent.append(node);return()=>node.remove();`;
  await c14SimpleMutation(live, "history-authority", addHistoricalArtifact("mission-r17-gate"), receipt);
  await c14SimpleMutation(live, "history-authority", addHistoryRow("unknown-session-id"), receipt);
  await c14SimpleMutation(live, "history-authority", addHistoricalArtifact("unknown-report-id"), receipt);
}
...
async function assertFullObjectInspect(live: Live, object: { type: string; id: string }, expectedCurrentIds: string[], expectedCurrentLinks: string[]): Promise<void> {
  await waitFor(`FULL_LINEAGE before ${object.type}:${object.id}`, async () => await evaluate<boolean>(live.endpoint, "document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='FULL_LINEAGE'&&document.querySelectorAll('.canvas-tile[data-qf-world-type]:not([hidden])').length===17&&document.querySelectorAll('.cable-path[data-qf-world-cable-kind]').length===21"), Date.now() + 5_000);
  await evaluate<boolean>(live.endpoint, `(() => { const tile=[...document.querySelectorAll('.canvas-tile[data-qf-world-type]')].find((node)=>node.dataset.qfWorldType===${JSON.stringify(object.type)}&&node.dataset.qfWorldId===${JSON.stringify(object.id)});if(!(tile instanceof HTMLElement))throw new Error('exact FULL object missing');tile.dispatchEvent(new PointerEvent('pointerdown',{bubbles:true,button:0,pointerType:'mouse',isPrimary:true}));return true;})()`);
  await waitFor(`FULL object Inspect ${object.type}:${object.id}`, async () => await evaluate<boolean>(live.endpoint, `(() => { const pane=document.querySelector('#dock-inspect-pane');const dock=document.querySelector('[data-dock-primary="INSPECT"]');const identity=pane?.querySelector('.dock-inspect-id')?.textContent??'';return document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='FULL_LINEAGE'&&dock instanceof HTMLElement&&!dock.hidden&&pane?.dataset.qfProjectionSubject===${JSON.stringify(object.id)}&&identity.includes(${JSON.stringify(object.id)})&&pane.querySelector('[data-qf-back-to-world]') instanceof HTMLElement;})()`), Date.now() + 5_000);
  await evaluate<boolean>(live.endpoint, "(() => { const back=document.querySelector('#dock-inspect-pane [data-qf-back-to-world]');if(!(back instanceof HTMLElement))throw new Error('FULL object Back missing');back.dispatchEvent(new PointerEvent('click',{bubbles:true,button:0,pointerType:'mouse',isPrimary:true}));return true;})()");
  await assertOrdinaryCanvas(live);
  const current = await openCurrentMission(live, expectedCurrentIds, expectedCurrentLinks); assert(!current.selected&&current.inspectorHidden, `stale selection or Inspect after FULL object Back: ${JSON.stringify(current)}`);
  await evaluate<boolean>(live.endpoint, "(() => { const full=document.querySelector('[data-qf-world-full]');if(!(full instanceof HTMLElement)||full.hidden)throw new Error('Show full lineage missing after current Mission');full.dispatchEvent(new PointerEvent('click',{bubbles:true,button:0,pointerType:'mouse',isPrimary:true}));return true;})()");
  await waitFor(`FULL_LINEAGE restored after ${object.type}:${object.id}`, async () => await evaluate<boolean>(live.endpoint, "document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='FULL_LINEAGE'&&document.querySelectorAll('.canvas-tile[data-qf-world-type]:not([hidden])').length===17&&document.querySelectorAll('.cable-path[data-qf-world-cable-kind]').length===21"), Date.now() + 5_000);
}

async function assertRelationshipInspect(live: Live, link: { kind: string; from_id: string; to_id: string }, expectedIds: string[], expectedLinks: string[], restoreState: "CURRENT_MISSION" | "FULL_LINEAGE" = "FULL_LINEAGE"): Promise<Json> {
  try {
    const proof = await evaluate<Json>(live.endpoint, `(async () => {
      ...
      const rect=(node)=>node.getBoundingClientRect();const center=(node)=>{const r=rect(node);return{x:(r.left+r.right)/2,y:(r.top+r.bottom)/2};};const controls=document.querySelector('#research-world-projection');const dock=document.querySelector('[data-dock-primary="INSPECT"]');const inspector=document.querySelector('#cable-inspector');const selected=document.querySelector('.cable-path--selected');const back=document.querySelector('[data-qf-back-to-world]');if(controls?.dataset.qfProjectionState!==${JSON.stringify(restoreState)}||!(dock instanceof HTMLElement)||dock.hidden||!(inspector instanceof HTMLElement)||inspector.hidden||!(selected instanceof SVGPathElement))throw new Error('relationship selection did not preserve ${restoreState} Dock Inspect');
      ...
      if(!(back instanceof HTMLElement))throw new Error('Back to world missing after relationship Inspect');back.click();await new Promise((resolve)=>setTimeout(resolve,100));if(controls?.dataset.qfProjectionState!=='ORDINARY_CANVAS')throw new Error('Back to world did not establish ORDINARY_CANVAS');return{kind:${JSON.stringify(link.kind)},from_id:${JSON.stringify(link.from_id)},to_id:${JSON.stringify(link.to_id)},label,paintedHeight:pr.height,endpointTolerance:true,unrelatedObstruction:true,restored:true};
    })()`);
    await assertOrdinaryCanvas(live); await openCurrentMission(live, expectedIds, expectedLinks); if (restoreState === "FULL_LINEAGE") { await evaluate<boolean>(live.endpoint, "document.querySelector('[data-qf-world-full]')?.click(); true"); await waitFor("FULL_LINEAGE after relationship Inspect", async () => await evaluate<boolean>(live.endpoint, "document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='FULL_LINEAGE'"), Date.now() + 5_000); } return proof;
  } catch (error) {
...
async function runLiveR17C14Proof(falsifier: Falsifier | null = null): Promise<boolean> {
  ...
  try {
    ...
    const beforeTerminalContinuation = outcomeSnapshot(kernelDb); assert(beforeTerminalContinuation === canonicalJson({ ticketRows: [], outcomeLinks: [], gradeRows: [], gradeOutgoing: [] }), "settlement state existed before terminal production continuation");
    const continuation = await waitFor("single terminal production review continuation", async () => { const attempts = dbRows(kernelDb, "SELECT attempt_id, outcome FROM qf_review_attempt WHERE action_kind = 'request_review' AND source_task_id = ? ORDER BY created_at, attempt_id", "task-r17-gate"); if (attempts.length > 1) throw new Error(`production journey created ${attempts.length} request_review attempts`); if (attempts.length !== 1 || attempts[0]?.outcome !== "admitted") return null; const reviews = dbRows(kernelDb, "SELECT task_id, critic_session_id, lifecycle FROM qf_review_task WHERE kind = 'review' AND source_task_id = ? ORDER BY created_at, task_id", "task-r17-gate"); if (reviews.length > 1) throw new Error(`production journey created ${reviews.length} review Tasks`); const review = reviews[0]; if (!review || review.lifecycle !== "completed") return null; const reviewTaskId = String(review.task_id ?? ""); const criticSessionId = String(review.critic_session_id ?? ""); const task = dbRows(kernelDb, "SELECT status FROM task WHERE id = ?", reviewTaskId)[0]; if (!reviewTaskId || !criticSessionId || task?.status !== "done") return null; const evaluations = dbRows(kernelDb, "SELECT id, findings_artifact_id, publication_report_id FROM evaluation WHERE review_task_id = ? AND json_extract(source_work, '$.run_id') = ? ORDER BY created_at, id", reviewTaskId, "run-r17-gate"); if (evaluations.length > 1) throw new Error(`production journey created ${evaluations.length} Evaluations for its review Task`); const evaluation = evaluations[0]; if (!evaluation) return null; const evaluationId = String(evaluation.id ?? ""); const findingsArtifactId = String(evaluation.findings_artifact_id ?? ""); const reportArtifactId = String(evaluation.publication_report_id ?? ""); if (!evaluationId || !findingsArtifactId || !reportArtifactId) return null; const findings = dbRows(kernelDb, "SELECT id FROM artifact WHERE id = ?", findingsArtifactId); const report = dbRows(kernelDb, "SELECT id FROM artifact WHERE id = ? AND kind = 'report'", reportArtifactId); const publication = dbRows(kernelDb, "SELECT publication_evaluation_id FROM qf_review_publication WHERE report_artifact_id = ? AND is_current = 1", reportArtifactId); if (findings.length !== 1 || report.length !== 1 || publication.length !== 1 || String(publication[0]?.publication_evaluation_id ?? "") !== evaluationId) return null; return { attemptId: String(attempts[0]?.attempt_id ?? ""), reviewTaskId, criticSessionId, evaluationId, findingsArtifactId, reportArtifactId }; }, Date.now() + 60_000);
    assert(outcomeSnapshot(kernelDb) === beforeTerminalContinuation, "settlement IPC occurred before terminal production continuation checkpoint"); console.log(`pre-r18-coherence: production-continuation=PASS request_review_attempt=${continuation.attemptId} settlement_before_checkpoint=0`);
    ...
    assert(initialWorld.objects.length === 15 && initialWorld.links.length === 17, `R17 pre-settlement world cardinality ${initialWorld.objects.length}/${initialWorld.links.length}`);
    for (const [type, id] of [["agent_session", executorSessionId], ["agent_session", continuation.criticSessionId], ["task", continuation.reviewTaskId], ["evaluation", continuation.evaluationId], ["artifact", continuation.findingsArtifactId], ["artifact", continuation.reportArtifactId]] as const) assert(initialWorld.objects.some((object) => object.type === type && object.id === id), `R17 production continuation object missing ${type}:${id}`);
    assert(initialWorld.links.some((link) => link.kind === "performed_by" && link.from_id === continuation.evaluationId && link.to_id === continuation.criticSessionId) && initialWorld.links.some((link) => link.kind === "gates" && link.from_id === continuation.evaluationId && link.to_id === continuation.reportArtifactId) && initialWorld.links.some((link) => link.kind === "produces" && link.from_id === continuation.criticSessionId && link.to_id === continuation.findingsArtifactId), "R17 production continuation exact identity links are missing");
    ...
    assert(dbRows(kernelDb, "SELECT attempt_id FROM qf_review_attempt WHERE action_kind = 'request_review' AND source_task_id = ?", "task-r17-gate").length === 1, "production journey did not retain exactly one request_review attempt through settlement");
    const { expected, ids } = resolveR17Bindings(kernelDb, oracle, directorSessionId, strategyId, continuation); compareResolvedWorld(world, expected); await assertSettledResultInspect(live, resultArtifactId);
    ...
    const expectedCurrentMissionIds = [...expectation.currentMissionIds].sort();
    ...
    await assertOrdinaryCanvas(live); const currentMissionProof = await openCurrentMission(live, expectedCurrentMissionIds, expectedDefaultLinks);
    assert(currentMissionProof.fullVisible === true && currentMissionProof.selected === false && currentMissionProof.inspectorHidden === true, `C14/current-Mission controls or overlay are wrong: ${JSON.stringify(currentMissionProof)}`);
    ...
    const defaultLinkProofs: Json[] = []; for (const link of defaultLinks) defaultLinkProofs.push(await assertRelationshipInspect(live, link, expectedCurrentMissionIds, expectedDefaultLinks, "CURRENT_MISSION"));
    const tileSpecs = tileReceiptSpecs(world, expectation.primaryIds);
    const defaultReceipt = await evaluate<{ facts: Record<string, boolean>; rows: Json[]; bounds: Json }>(live.endpoint, defaultReceiptExpression(tileSpecs, expectedCurrentMissionIds, expectedDefaultLinks, expectation.stages));
    ...
    console.log(`pre-r18-coherence: c14/current-Mission=PASS ${JSON.stringify({objects:currentMissionProof.visible.length,links:currentMissionProof.links.length,bounds:defaultReceipt.bounds})}`);
    if (falsifiesC14(falsifier, "default-projection")) await runDefaultMutationMatrix(live, tileSpecs, expectedCurrentMissionIds, expectedDefaultLinks, expectation.stages, defaultLinks);
    ...
    assert(localProof.state === "CURRENT_MISSION" && localProof.dockVisible === true && localProof.back === true, `C14/local-lineage did not open Dock Inspect: ${JSON.stringify(localProof)}`);
    ...
    await assertOrdinaryCanvas(live); const backMissionProof = await openCurrentMission(live, expectedCurrentMissionIds, expectedDefaultLinks); const backReceipt = backMissionProof.fullVisible && !backMissionProof.selected;
    assert(backReceipt, "Back to world did not restore ordinary Canvas before exact current Mission navigation");
    ...
    assert(String(world.current_report_id ?? "") === ids.report_artifact_id, "C14/history-authority current Report differs from Kernel world authority");
    const expectedHistoricalReports = ((world as Json).report_ids instanceof Array ? (world as Json).report_ids as unknown[] : []).map(String).filter((id) => id !== ids.report_artifact_id).sort();
    const historyExpression = historyReceiptExpression([directorSessionId, executorSessionId, ids.critic_session_id], expectedHistoricalReports, ids.report_artifact_id, "mission-r17-gate");
    const historyReceipt = await evaluate<{ ok: boolean; ledger: string[]; sessions: string[]; expectedSessions: string[]; historicalReports: string[]; expectedHistoricalReports: string[]; reportStatus: string; reportMarkers: string; missionCurrent: boolean }>(live.endpoint, historyExpression);
    ...
    if (falsifiesC14(falsifier, "history-authority")) await runHistoryMutationMatrix(live, `(${historyExpression}).ok`, ids.report_artifact_id);
    await captureState(live, "07-completed-world");
    const expectedSavedHash = createHash("sha256").update(world.objects.filter((object)=>object.type!=="agent_session").map((object)=>`research\u0000ontology:${object.type}:${object.id}`).sort().join("\n")).digest("hex");
    await closeLive(live); live = await launch(root, kernelDb, artifactRoot, appRoot); await openColdReopenedMission(live, expectedSavedHash, expectedCurrentMissionIds, expectedDefaultLinks); await assertSettledResultInspect(live, resultArtifactId); await openCurrentMission(live, expectedCurrentMissionIds, expectedDefaultLinks); await setDockMode(live, "START"); await captureState(live, "08-reopened-world"); await setDockMode(live, "CATALOG"); await captureState(live, "09-dock-catalog"); await assertReopenedParticipantInvariant(live, kernelDb, directorSessionId, executorSessionId, ids.critic_session_id, ids.review_task_id); await closeParticipantInspectors(live); await prepareProjectionObjectCapture(live, "artifact", String(ids.result_artifact_id)); await captureState(live, "12-selected-artifact"); await evaluate<boolean>(live.endpoint, "document.querySelector('[data-qf-back-to-world]')?.click(); true"); await assertOrdinaryCanvas(live); await openCurrentMission(live, expectedCurrentMissionIds, expectedDefaultLinks); await prepareProjectionObjectCapture(live, "evaluation", String(ids.evaluation_id)); await captureState(live, "13-selected-evaluation"); await evaluate<boolean>(live.endpoint, "document.querySelector('[data-qf-back-to-world]')?.click(); true"); await assertOrdinaryCanvas(live);
    await closeLive(live); live = await launch(root, kernelDb, artifactRoot, appRoot); await openColdReopenedMission(live, expectedSavedHash, expectedCurrentMissionIds, expectedDefaultLinks); await assertSettledResultInspect(live, resultArtifactId); await openCurrentMission(live, expectedCurrentMissionIds, expectedDefaultLinks); await setDockMode(live, "START"); await evaluate<boolean>(live.endpoint, "(() => { const button=document.querySelector('[data-qf-world-full]'); if(!(button instanceof HTMLElement))throw new Error('Show full lineage missing'); button.click(); return true; })()"); await waitFor("FULL_LINEAGE overview", async () => await evaluate<boolean>(live!.endpoint, "document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='FULL_LINEAGE'"), Date.now() + 5_000);
    if (falsifiesC14(falsifier, "back-to-world")) await c14SimpleMutation(live, "back-to-world", `const node=document.querySelector('[data-qf-world-back]');if(!(node instanceof HTMLElement))throw new Error('Canvas Back to world missing');const had=node.hasAttribute('data-qf-world-back');node.removeAttribute('data-qf-world-back');node.click();return()=>{if(had)node.setAttribute('data-qf-world-back','');};`, "document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='ORDINARY_CANVAS'");
    ...
    await assertOrdinaryCanvas(live); const fullBackMission = await openCurrentMission(live, expectedCurrentMissionIds, expectedDefaultLinks); const fullBackReceipt = fullBackMission.fullVisible && !fullBackMission.selected;
    passC14SimpleReceipt("back-to-world", fullBackReceipt);
    await evaluate<boolean>(live.endpoint, "(() => { const button=document.querySelector('[data-qf-world-full]'); if(!(button instanceof HTMLElement))throw new Error('Show full lineage missing after current Mission'); button.click(); return true; })()"); await waitFor("FULL_LINEAGE restored", async () => await evaluate<boolean>(live!.endpoint, "document.querySelector('#research-world-projection')?.dataset.qfProjectionState==='FULL_LINEAGE'"), Date.now() + 5_000); await captureState(live, "14-most-cable-dense-region"); await captureManifestReceipt();
    const objectKeys = expected.objects as Array<{ type: string; id: string }>; const linkKeys = expected.links as Array<{ kind: string; from_id: string; to_id: string }>; const measurement = await evaluate<{ objectCount: number; linkCount: number; canvas: Json; tiles?: Json[]; paths?: Json[] }>(live.endpoint, fullGeometryExpression(objectKeys, linkKeys)); assert(measurement.objectCount === 17 && measurement.linkCount === 21, "C14/full-lineage did not paint all 17 objects and 21 links"); const objectProofs: string[] = []; for (const object of objectKeys) { await assertFullObjectInspect(live, object, expectedCurrentMissionIds, expectedDefaultLinks); objectProofs.push(`${object.type}:${object.id}`); } console.log(`pre-r18-coherence: c14/object-inspect=PASS objects=${objectProofs.length}`);
    const relationshipProofs: Json[] = []; for (const link of linkKeys) relationshipProofs.push(await assertRelationshipInspect(live, link, expectedCurrentMissionIds, expectedDefaultLinks)); console.log(`pre-r18-coherence: c14/relationship-inspect=PASS links=${linkKeys.length}`);
    ...
    fullReceipt.facts["object-select-inspect"] = fullReceipt.facts["object-select-inspect"] === true && objectProofs.length === objectKeys.length;
    ...
    const evaluationLocal = await waitFor("LOCAL Evaluation lineage", async () => await evaluate<{ state: string; dockVisible: boolean; back: boolean; dim: string[]; normal: string[] }>(live!.endpoint, localStateExpression({kind:"object",id:ids.evaluation_id})), Date.now() + 5_000); assert(evaluationLocal.state === "FULL_LINEAGE" && evaluationLocal.dockVisible === true && evaluationLocal.back === true, `C14/local-lineage Evaluation Inspect failed: ${JSON.stringify(evaluationLocal)}`); await evaluate<boolean>(live.endpoint, "document.querySelector('[data-qf-back-to-world]')?.click(); true"); console.log(`pre-r18-coherence: c14/local-evaluation=PASS ${JSON.stringify({normal:evaluationLocal.normal.length,dim:evaluationLocal.dim.length})}`);

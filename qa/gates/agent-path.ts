/**
 * WO-006c headless E2E: spawn → stream → tool → artifact → cancel/concurrency
 * → orphan check → reconciliation. Bun + openKernel + AgentOs (no Electron).
 *
...
} from "../../tools/runtime-proof/node_modules/@rivet-dev/agentos-core/dist/index.js";
import {
  execute,
  listAgentSessions,
  listArtifacts,
...
async function resolveModule(specifier: string): Promise<string> {
  // Prefer tools/runtime-proof install of agentos-core.
  const candidates = [
    join(REPO, "tools/runtime-proof/node_modules", specifier),
    join(REPO, "collab-electron/node_modules", specifier),
    join(REPO, "node_modules", specifier),
  ];
...
async function runOne(
  ...
  if (!opts.skipPublish) {
    const dir = join(REPO, "qa/.tmp-agent-path");
...
export async function runAgentPathGate(): Promise<{ ok: boolean }> {
  // Ensure agentos resolves (cold: use runtime-proof node_modules).
  await resolveModule("@rivet-dev/agentos-core");
  if (!existsSync(AOSPKG)) {
    console.error("agent-path FAIL: missing", AOSPKG, "— run pack-agent");
    return { ok: false };
  }
  ...
  try {
    // Concurrent pair (or serialized for falsify).
    let a: RunOut;
    let b: RunOut;
    if (serialize) {
      ...
      // Concurrency: both non-terminal at some instant — poll Kernel.
      ...
      await poll;
      if (!sawConcurrent) {
        // interleaved chunks also count
        if (a.chunks > 0 && b.chunks > 0) sawConcurrent = true;
      }
      if (!sawConcurrent) {
    ...
    if (corruptId) {
      ...
      // Adoption broken — gate must fail
    ...
    if (a.status !== "cancelled" && a.stopReason !== "cancelled") {
      // After close, status is closed; check events for cancelled
      const cancelledEvent = db
        .query(
          `SELECT type FROM events WHERE object_id = ? AND type = ?`,
        )
    ...
    // Reconciliation: synthetic starting + running
    execute(db, "create_agent_session", {
...
if (import.meta.main) {
  // Resolve agentos from runtime-proof when not hoisted.
  const corePath = join(
    REPO,
    "tools/runtime-proof/node_modules/@rivet-dev/agentos-core",
  );
  if (existsSync(corePath)) {
    // Bun resolves via import map-less node_modules walk from cwd — run from repo.
  }
  void pathToFileURL;

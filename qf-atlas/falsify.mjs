...
import { REPO, rel, walk, IS_TEST, extractWires } from "./extract.mjs";
...
const GOVERNED_REVIEW_BAIT_PRELOAD = "collab-electron/src/preload/zz-falsify-governed-review-bait.ts";
const GOVERNED_REVIEW_BAIT_RENDERER = "collab-electron/src/windows/agent-chat/src/zz-falsify-governed-review-bait.ts";
const GOVERNED_REVIEW_BAIT = {
  ...
  [GOVERNED_REVIEW_BAIT_PRELOAD]:
    `import { contextBridge, ipcRenderer } from "electron";\n` +
    `contextBridge.exposeInMainWorld("qfFalsifyApi", {\n` +
    `  governedReviewBypass: (id: string) => ipcRenderer.invoke("qf:falsify:governed-review-bypass", id),\n` +
    `});\n`,
  [GOVERNED_REVIEW_BAIT_RENDERER]:
    `export function runGovernedReviewBypass(id: string) {\n` +
    `  return window.qfFalsifyApi.governedReviewBypass(id);\n` +
    `}\n`,
};
...
function artifactDigest(snapshot) {
  const hash = createHash("sha256");
  for (const p of ARTIFACTS) {
    hash.update(p);
    hash.update(snapshot.get(p) ?? Buffer.from("<missing>"));
  }
  return hash.digest("hex");
}

function currentArtifactSnapshot() {
  return new Map(ARTIFACTS.map((p) => {
    const abs = join(REPO, p);
    return [p, existsSync(abs) ? readFileSync(abs) : null];
  }));
}
...
function withGovernedReviewBait(assertFn) {
  ...
  const artifactsBefore = currentArtifactSnapshot();
  const artifactHashBefore = artifactDigest(artifactsBefore);
  const written = [];
  let result = [false, "bait assertion did not run"];
  let baitRedCount = 0;
  let baitRedIds = [];
  let baitWire = null;
  let baitTransportWire = null;
  let baitContract = false;
  ...
  try {
    ...
    baitRedIds = baitRedRows.map((p) => p.id).sort();
    baitWire = (baitModel.wires ?? []).find((x) => x.channel === "qf:falsify:governed-review-bypass");
    const appFiles = walk(REPO, [], true).filter((f) => {
      const p = rel(f);
      return p.endsWith(".ts") || p.endsWith(".tsx") || p.endsWith(".js") || p.endsWith(".jsx");
    });
    baitTransportWire = extractWires(
      appFiles.filter((f) => rel(f).startsWith("collab-electron/src/main/")),
      appFiles.filter((f) => rel(f).startsWith("collab-electron/src/preload/")),
      appFiles.filter((f) => rel(f).startsWith("collab-electron/src/windows/") || rel(f).startsWith("collab-electron/packages/")),
    ).wires.find((x) => x.channel === "qf:falsify:governed-review-bypass");
    const baitTables = baitRedRows.map((p) => p.table).sort();
    const allTransportHopsLive = baitWire?.hops
      ?.filter((h) => h.layer !== "kernel")
      ?.every((h) => h.present === true);
    baitContract = baitTransportWire?.status === "live"
      && baitTransportWire?.breakAt == null
      && allTransportHopsLive
      && baitWire?.hop4?.kind === "cheats"
      && baitWire?.status === "cheats"
      && baitWire?.breakAt === "kernel"
      && baitRedRows.length === 2
      && baitTables.join(",") === "links,task"
      && baitRedRows.every((p) => p.confidence === "high"
        && p.governance === "violation"
        && p.reachProof?.corroborated);
    result = baitContract
      ? assertFn(baitModel, {
        baitRows, baitRed, baitRedRows, baitRedIds, baitWire, baitContract,
        files: GOVERNED_REVIEW_BAIT,
      })
      : [false, `bait contract failed: transport=${baitTransportWire?.status ?? "missing"}/${baitTransportWire?.breakAt ?? "none"} ` +
        `wire=${baitWire?.status ?? "missing"}/${baitWire?.breakAt ?? "none"}/${baitWire?.hop4?.kind ?? "none"} ` +
        `redIds=${baitRedIds.join("|")} redCount=${baitRedRows.length}`];
  } finally {
  ...
  const artifactsAfter = currentArtifactSnapshot();
  const artifactHashAfter = artifactDigest(artifactsAfter);
  const baitFilesRestored = [
    GOVERNED_REVIEW_BAIT_SOURCE,
    GOVERNED_REVIEW_BAIT_HANDLER,
    GOVERNED_REVIEW_BAIT_PRELOAD,
    GOVERNED_REVIEW_BAIT_RENDERER,
  ].every((p) => !existsSync(join(REPO, p)));
  ...
  return [Boolean(ok) && restoredRedCount === 0 && clean,
    `${detail}; baitTransport=${baitTransportWire?.status ?? "missing"}/${baitTransportWire?.breakAt ?? "none"} ` +
    `baitWire=${baitWire?.status ?? "missing"}/${baitWire?.breakAt ?? "none"}/${baitWire?.hop4?.kind ?? "none"} ` +
    `baitRedIds=${baitRedIds.join("|")} baitHardRed=${baitRedCount} ` +
    `restoredHardRed=${restoredRedCount} artifactHashBefore=${artifactHashBefore} ` +
    `artifactHashAfter=${artifactHashAfter} sourceRestored=${baitFilesRestored} restored=${clean}`];
}

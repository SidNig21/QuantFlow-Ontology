...
f09-reader-verdict: **YES / YES — latest same Reader task `01a04da4-185a-7943-9a1b-1d36735f5cad` accepted exactly one bounded native-input sign product+gate repair; same Builder resumes and one fresh independent Verifier remains required**
f09-surface: **only `collab-electron/src/main/ipc-browser.ts` browser:scroll uses `const wc = getWc(webContentsId); await wc.sendInputEvent({ type: "mouseWheel", x: 0, y: 0, deltaX: -x, deltaY: -y }); return {};`, removing only the debugger-CDP wheel dependency, plus `qa/gates/golden-g10-canvas-runtime.ts` using existing normal `rpcCall` default 5000ms boundary with no no-wait helper and unchanged post-completion `wait(50)`; preserve the guest webContentsId, real Canvas → Main → browser route, all other dirty G10 work, and the prior gate contract**
f09-proof: **current native signs (`+800` for the downward test at `scrollTop=0`) remain red because the wheel clamps upward; restore only `deltaX: -x, deltaY: -y` and the same target route yields `after > before` within unchanged post-command 50ms; deltaY=0 or removed dispatch independently remains red, overflow remains red, and cleanup remains zero**
...
| F09 | on the isolated live fixture retain the standards/readiness/`document.scrollingElement` precondition, verify a nonzero surface, then use the current native signs as the first causal bait and independently use zero-delta, removed-dispatch, and overflow baits | current `deltaY: +800` at `scrollTop=0` attempts upward and clamps, leaving `after === before`; independently setting `deltaY: 0` or removing wheel dispatch leaves `after === before`, and overflow poisoning exits nonzero | in `browser:scroll`, use `const wc = getWc(webContentsId); await wc.sendInputEvent({ type: "mouseWheel", x: 0, y: 0, deltaX: -x, deltaY: -y }); return {};`; remove only the debugger-CDP wheel dependency; use existing normal `rpcCall` default 5000ms boundary with no no-wait helper; start unchanged `wait(50)` only after RPC completion; preserve the guest webContentsId, native user-visible input, real Canvas → Main → browser route, `after > before`, standards/readiness/`document.scrollingElement`, overflow red, no DOM shortcut, and cleanup zero |
...
## Current F09 finite native-input sign repair — same Builder resumes

The latest finite Reader evidence task
`01a04da4-185a-7943-9a1b-1d36735f5cad` returned **YES / YES** for exactly one
bounded native-input sign repair. The awaited-CDP hypothesis is superseded: the

...
status: G10 F09 RUNTIME READINESS CORRECTION AUTHORIZED — same Reader reread required before same Builder; fresh independent Verifier required
kind: Golden Baseline Phase 2 bounded current-product coherence group
owner: Router
depends: G9 CLOSED / PASS WITH G12 INHERITED RED
build-authority: **NO BUILDER IS OPEN — the same Reader task `01a04da4-185a-7943-9a1b-1d36735f5cad` must reread this order and NEXT.md and return YES / YES before the same Builder resumes; then exactly one gate/runtime-readiness correction in `qa/gates/golden-g10-canvas-runtime.ts` is authorized, the noncausal focus-line repair must be reverted to starting-HEAD bytes, the full gate rewrite and retained fixture correction must be preserved, and one fresh independent Verifier still decides G10**
...
f09-reader-verdict: **NO / NO — the focus-only repair is noncausal; one finite gate/runtime-readiness correction is required; same Reader must reread this order and NEXT.md and return YES / YES before the same Builder resumes, and one fresh independent Verifier remains required**
f09-surface: **only `qa/gates/golden-g10-canvas-runtime.ts`: within the existing accepted readiness envelope wait for host webview `getBoundingClientRect().width > 0 && height > 0` and guest `innerWidth > 0 && innerHeight > 0`, and measure `document.scrollingElement` exclusively for `scrollHeight`, `clientHeight`, and `scrollTop`; remove/revert the uncommitted `tileManager.focusCanvasTile(params.tileId)` line in `collab-electron/src/windows/shell/src/canvas-rpc.js` to starting-HEAD bytes; `collab-electron/src/main/ipc-browser.ts` must remain byte-identical to starting HEAD**
f09-proof: **retained doctype/standards and positive-range precondition; on verified nonzero surface, independently remove only wheel dispatch or set `deltaY: 0` so `after === before` is red; restore the exact existing payload `{type: "mouseWheel", x: 0, y: 0, deltaX: 0, deltaY: 800}` so `after > before` is green; overflow remains red and cleanup remains zero**
f09-reread: **REQUIRED — same Reader task `01a04da4-185a-7943-9a1b-1d36735f5cad` must reread amended NEXT.md and this order and return YES / YES before same Builder**
...
repair-authority: **HISTORICAL/RETAINED — prior same-meaning fixture/driver and lineage repairs are complete for this adjudication; current authority is the founder-reauthorized F04 lifecycle-order correction below**
repair-surface: **HISTORICAL/RETAINED — prior G10 surfaces remain read-only; the current F04 surface is named below**
...
lineage-repair-authority: **HISTORICAL/RETAINED — the accepted two-artifact/pre-admission lineage contract is preserved; current authority is the founder-reauthorized F04 lifecycle-order correction below**
lineage-repair-surface: **HISTORICAL/RETAINED — prior helper/gate/test surfaces are read-only under this amendment; the current F04 surface is named below**
...
ordinary-repair-authority: **HISTORICAL/RETAINED — prior ordinary-Canvas repair is complete for this adjudication; no current ordinary repair is authorized**
ordinary-repair-surface: **HISTORICAL/RETAINED — prior ordinary-Canvas repair remains read-only; the current F04 participant-projection amendment narrows all active repair authority to the gate file below**
...
browser-repair-authority: **HISTORICAL/RETAINED — prior browser-gate repair is complete for this adjudication; no current browser repair is authorized**
browser-repair-surface: **HISTORICAL/RETAINED — prior browser-readiness repair remains read-only; the current F04 authority is limited to participant parity in the gate file below, with no timeout/policy change**
...
mission-repair-authority: **HISTORICAL/RETAINED — the exact `return null` gate correction and current-Mission HISTORY projection repair remain binding; current authority is the founder-reauthorized F04 lifecycle-order correction below**
...
mission-reopen-reader-task: `01a04bf8-eaba-7e53-8563-95d85d0f078a`
mission-reopen-reader-authority: `8c594f17cb7881fd4bb81c6e059d0157e946f549`
mission-reopen-reader-verdict: **YES / YES — repeated reopening of the active durable Mission is part of the accepted visible-HISTORY consumer path; the bounded allowlist and red/green proof below have one finite meaning**
mission-reopen-classification: **A primary — G10 product projection defect; the prior generic `RPC timeout: app.ui.evaluate` was D insufficient diagnostics and is not a product defect finding**
f06-adjudication-task: `01a04c4c-afc5-72e1-b554-6c45a46482e9`
f06-adjudication-authority: `a7a98ceec20140581b7eb72cfaa783564dc367f9`
f06-classification: **HISTORICAL/RETAINED — A primary — G10 product projection defect; D secondary — old R16 cardinality wording; projection repair is now retained/proven by focused 14-object/16-link tests**
f06-canonical-produces-invariant: **Run → result_set Artifact; Executor AgentSession → worker trajectory Artifact; Critic AgentSession → findings Artifact; no Task → Artifact producer relation**
f06-durable-missing-tuple: **HISTORICAL/REPAIRED — the Kernel contains `produces(synthetic-worker-37085aa4-a3bc-4c0e-af2b-cb4cce231066, af5da2f0d1b98edcf6e90df9246094a1a7665ccaa03dc8489083c14b4393d7c5)`; focused tests prove 14 objects/16 links and retain that exact tuple in the read-only projection**
f06-renderer-receipt-adjudication-task: `01a04c68-4710-7fe3-9b92-2a1267d85013`
f06-renderer-receipt-adjudication-authority: `58c7aecc72e0b7704b190773de536a0d83d620f9`
f06-renderer-receipt-classification: **D primary — gate/consumer-boundary ambiguity; distinct from the earlier A-primary projection assertion, so the same-assertion-twice stop is not reached**
f06-renderer-receipt-result: **the real pointer/Inspect consumer path is required before asserting the exact Run→result_set Artifact `.qf-world-relation` row; `run=true`, `relation=false`, `missing=true` is not a new projection failure**
f06-reader-task: `01a04c5b-5104-7a10-a31b-4a28c4c0a340`
f06-reader-requirement: **HISTORICAL/RETAINED — the F06 consumer proof is complete; the current same-Reader requirement is the F04 compact Canvas participant seam and lifecycle contract below**
f06-repair-authority: **HISTORICAL/RETAINED — the F06 gate-only consumer proof is complete; current authority is the founder-reauthorized F04 lifecycle-order correction below**
f06-repair-surface: **HISTORICAL/RETAINED — only the prior F06 renderer-receipt proof was authorized; no current F06 edit is authorized under the founder-reauthorized F04 lifecycle-order correction**
f04-adjudication-task: `01a04c7e-d62c-7ee2-8285-f080b213f744`
f04-classification: **D primary — gate/consumer-boundary selector mismatch; A, B, and E unsupported; C secondary only for the separate post-completion lifecycle sequencing defect**
f04-reader-task: `01a04c5b-5104-7a10-a31b-4a28c4c0a340`
f04-reader-requirement: **WAIVED FOR THIS FOUNDER REAUTHORIZATION — no new semantic Reader is required for the one mechanical same-meaning lifecycle-order correction; the accepted F04 seam and contract remain unchanged**
f04-repair-authority: **AUTHORIZED ONCE — exactly one bounded lifecycle-order correction is authorized in `qa/gates/golden-g10-canvas-runtime.ts`; after the twice-red stop, no second correction is authorized without new founder direction**
f04-repair-surface: **only `qa/gates/golden-g10-canvas-runtime.ts`; all product behavior, Canvas, Kernel, Ontology, identity, state, parity, timeouts, cleanup, falsifier meaning, F06, and scope remain unchanged**
f04-lifecycle-order: **seed and reveal the real Mission world before pre-finalization participant parity; the old ordering must fail for the identified causal reason, and corrected ordering must reach the same real Mission world and exact participant identity/state without synthetic substitution**
f04-lifecycle-acceptance: **focused suite plus one complete G10 gate; cleanup zero; green freezes an immutable candidate for one fresh independent Verifier; F04 red again stops; G11 remains closed**
...
diagnostic-adjudication-classification: **HISTORICAL/SUPERSEDED — D primary — insufficient diagnostics; C was only a conditional mechanical F11 hypothesis; A, B, and E were unsupported**
diagnostic-adjudication-result: **HISTORICAL/SUPERSEDED — the generic `RPC timeout: app.ui.evaluate` did not identify the post-HISTORY phase; the later repeated Mission reopening failure is separately classified A primary by the same Reader**
barrier-reader-task: `01a04a11-0b55-79b3-b6c0-55285177dd55`
barrier-reader-requirement: **SATISFIED — the same Reader task `01a04bf8-eaba-7e53-8563-95d85d0f078a` returned YES / YES at authority `8c594f17cb7881fd4bb81c6e059d0157e946f549`; no new semantic Reader is required for this current-Mission projection amendment**
barrier-repair-authority: **HISTORICAL/SUPERSEDED — the diagnostic-only hold is closed by the Reader-accepted A-primary current-Mission projection amendment below**
barrier-repair-surface: **HISTORICAL/SUPERSEDED — prior diagnostics only; current repair surface is exactly `collab-electron/src/windows/shell/src/renderer.js` and `qa/gates/golden-g10-canvas-runtime.ts` as specified below**
...
| F09 | on the isolated live fixture remove only the doctype, then poison the verified nonzero surface by independently removing only wheel dispatch or setting `deltaY: 0`, then restore each exact input/readiness condition | doctype removal observes standards/positive-range failure; after exact restoration, either wheel-dispatch bait independently leaves `after === before` or zero-delta bait independently leaves `after === before`, and each exits nonzero; overflow poisoning also exits nonzero | within the existing accepted readiness envelope, host webview `getBoundingClientRect().width > 0 && height > 0` and guest `innerWidth > 0 && innerHeight > 0` are observed; `document.scrollingElement` alone supplies `scrollHeight`, `clientHeight`, and `scrollTop`; the exact existing `{type: "mouseWheel", x: 0, y: 0, deltaX: 0, deltaY: 800}` payload traverses the real Canvas → Main → renderer/preload → browser IPC route with no-wait dispatch, exact 50ms interval, unchanged `after > before`, and cleanup zero; exact restoration returns green |
...
Builder was authorized. That prior Builder opening is superseded by the F06
Reader hold below. The Builder must preserve G9 Report authority, the
...
current Builder is held pending the fresh F06 Reader YES / YES below. All
prior G10/G9/G12/R18 boundaries, the accepted browser and Mission routes,
two-Artifact/pre-admission lineage, F14a/F14b ownership, and every other
Golden-group boundary remain unchanged. No product or semantic red outside
the current F06 authority may be repaired.

## Historical post-HISTORY diagnostic-only measurement — superseded by current-Mission projection adjudication
...
## Historical current-Mission HISTORY projection adjudication — prior Reader accepted; superseded by F06 hold
...
## Historical F06 two-artifact projection/cardinality adjudication — projection repair retained; superseded by renderer-receipt hold
...
## Current F06 renderer-receipt consumer adjudication — same Reader reread required before Builder
...
## Current F04 participant-projection adjudication — founder-reauthorized lifecycle-order correction
...
## Current F09 finite runtime-readiness correction — same Reader reread required

The latest finite Reader evidence task
`01a04da4-185a-7943-9a1b-1d36735f5cad` returned **NO / NO** and superseded the
noncausal focus-only hypothesis. The existing CDP wheel payload is already
valid; the current red is an invalid runtime-readiness/scroll-root observation
because the measured surface reports a zero-size guest viewport. The exact
accepted F09 invariant remains observable guest-page scrolling through the real
Canvas → Main → renderer/preload → browser IPC path within the existing 50ms
check.

Authorize exactly one gate/runtime-readiness correction in
`qa/gates/golden-g10-canvas-runtime.ts`:

1. Within the existing accepted readiness envelope, wait for host webview
...
The same Reader must reread this amended `NEXT.md` and this order and return
**YES / YES** before the same Builder resumes. One fresh independent

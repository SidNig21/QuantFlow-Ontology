...
## Current F09 finite deterministic guest-scroll repair — same Builder resumes

The latest finite Reader evidence task
`01a04da4-185a-7943-9a1b-1d36735f5cad` returned **A** for exactly one bounded
deterministic guest-scroll repair. The awaited-CDP hypothesis is superseded: the
debugger wheel command does not resolve within the normal RPC boundary and
produces no after sample. F09 still protects user-visible guest-page scrolling
through the target webContents on the real Canvas → Main → browser route.
...
2. In the existing `browser:scroll` branch of
   ...
   Await `wc.executeJavaScript` with `userGesture: false` on a fixed closure:
   require `document.scrollingElement` or throw, add numeric `x` to its
   `scrollLeft`, add numeric `y` to its `scrollTop`, and return `true`; then
   return `{}` from the handler. Serialize only the validated numeric values
   and accept no arbitrary script input. This is the sole exception to the
   prior DOM-shortcut prohibition, and applies only to `browser:scroll`.

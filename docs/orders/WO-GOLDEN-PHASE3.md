...
Execution source `9ee5c688` remains immutable rejected ownership RED evidence. Its bounded repair descendant `9721db6d22e789715e1c1780c65dc36aef53ccac` (tree `5363a8332c500fec5b94aad80f79c2654e07f2c7`, sole parent `28dd91f8e2c1ebc92e0afef0974bad3a1b06a4f9`) is a second immutable rejected clean-topology RED: Electron owns the packaged consumer, but the release verifier installs only Electron before `qa/windows-unit.ts`, so direct source execution of `species/hermes/src/host-acp-client.ts` cannot resolve its own declared SDK from an empty archive. [The exact identities and blobs are frozen here](evidence/golden-baseline/phase3/REJECTED-EXECUTION-SOURCE-9721DB6D.md). Neither rejected source is amended, called candidate, or reused for P09–P17.
...
The additional authorized delta is only `qa/verify-release.ts` and `qa/verify-release.test.ts`: the verifier performs frozen installs with explicit cwd in `collab-electron` and `species/hermes`, in that order, both before `qa/windows-unit.ts`; the production change is only the minimal cwd type extension required by that stage representation. The test changes only the exact stage order/count assertions and adds the Hermes-install omission falsifier. `species/hermes/bun.lock` must remain byte-identical to rejected source `9721db6...` unless the isolated frozen install proves actual lock drift; `collab-electron/bun.lock` remains the new lock owning the Electron row. P09–P17 run only from a fresh archive of the new source. Byte equality against `9721db6...` is mandatory outside these verifier/test changes, permitted evidence, and unconditional generated Atlas outputs.
...
The focused repair proof is fail-capable: (1) isolated archive with only the Electron frozen install reproduces the clean-topology RED; (2) both frozen installs before `windows-unit` are GREEN; (3) omitting the Hermes install stage REDs the verifier test and focused archive proof; (4) omitting Electron ownership REDs even when Hermes or ambient state contains the SDK. Then `bun qa/windows-unit.ts`, build/package/first launch/normal shutdown/relaunch must pass. A credential-free fake executable must complete the real host-ACP bridge handshake, and native-TUI package admission plus shutdown must pass. These prove install orchestration and ownership, not new behavior.
...
- any dependency/version change beyond the two preserved exact SDK declarations and their named locks, any third/root declaration, resolver alias/`NODE_PATH`, source relocation, service, credential, network update, signing path, trade/bet execution, R18 capability, unsupported platform, or broader architecture change would be required;

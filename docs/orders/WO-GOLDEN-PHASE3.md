...
Execution source `9ee5c688` is immutable rejected RED evidence: isolated `windows-unit` produced `59 pass / 1 fail` because Electron lacked a direct runtime owner for `@agentclientprotocol/sdk`. It is never amended, called candidate, or reused for P09–P17.

After Reader YES / YES, the Builder creates a new clean `PHASE3_EXECUTION_SOURCE_SHA` whose exact parent is this amendment authority and whose enumerated delta is limited to `collab-electron/package.json`, `collab-electron/bun.lock`, the existing Phase-3 gate/registration, Phase-3 evidence, and Atlas outputs generated unconditionally after the repair. The package change is exactly direct runtime `@agentclientprotocol/sdk` version `0.18.2`; host-ACP compatibility and all behavior remain unchanged. P09–P17 run only from a fresh `git archive` of the new execution source. Byte equality against `9ee5c688` is mandatory outside the enumerated repair/evidence/Atlas delta.
...
The focused repair proof is fail-capable. With only the Electron dependency declaration omitted while the sibling Hermes package or ambient machine still contains the SDK, the Phase-3 ownership check must RED before runtime tests. Green requires the exact Electron declaration and lock closure from an isolated empty-cache frozen install. Then `bun qa/windows-unit.ts` must move from the recorded `59 pass / 1 fail` RED to all green; build/package/first launch/normal shutdown/relaunch must pass. A credential-free fake executable must complete the real host-ACP bridge handshake, and native-TUI package admission plus shutdown must pass. These are ownership/closure proofs, not new behavior.

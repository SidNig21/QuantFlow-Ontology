...
- **P14-A — live capture.** One fresh successful production Bovada NFL capture creates the Kernel source Artifact plus exact `market_event`, related `instrument`, and timestamped `quote` objects/links. Record source identity, observed/source timestamps, content hash, admitted IDs, and exact cardinality. A separately forced capture failure must produce no fabricated market objects or links. Network refusal alone does not satisfy P14-A; one successful live capture is mandatory.
- **P14-B — real Director turn.** One fresh non-synthetic turn launches the packaged Windows app's default Hermes Research Director profile and records durable launch/session/turn identity and exact output bytes/hash. Provider and model identity must be independently proved from a trusted runtime receipt. If that identity cannot be independently proved, record exactly `provider-model-identity-unprovable`, make no Kimi or other provider/model claim, and accept only the honest identity-refusal branch under the current claim; a synthetic or self-reported identity never passes.
...
- **P14-D — missing live composition refusal.** Attempt the unsupported live capture→Dataset→Technique composition through the current product and require an explicit refusal that names the missing R18 seam. Compare exact pre/post Kernel sets and require zero delta for Dataset, Run, result/report Artifact, Evaluation, and Report; capture objects from P14-A remain intact. Record no trade/bet. This refusal proves only the missing seam and keeps R18 open/frozen; it never counts as successful research composition.

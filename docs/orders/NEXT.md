# NEXT — R18 GOLDEN BASELINE / PHASE 2 CLOSED / PHASE 3 CABLE LABEL REPAIR

status: G1–G12 CLOSED / PHASE 2 ACCEPTED / PHASE 3 BASE `f0d06348` / PRESERVED AUTHORIZED DIRTY SET / C14 CABLE-LABEL TWO-FILE REPAIR OPEN / R18 CLOSED
...
builder-authority: **OPEN TO RESUME THE EXISTING AUTHORIZED SIX-PATH DIRTY SET AND ADD ONLY `collab-electron/src/windows/shell/src/cable-overlay.js` PLUS `collab-electron/src/windows/shell/src/cable-overlay.test.ts` — fresh Reader task `01a05212-f0e4-7101-a0d9-8e59df8a3f08` adjudicated a real C14 label-placement product defect; sample rendered path fractions `[.50,.375,.625,.25,.75]`, try existing offsets, exclude only exact endpoint rectangles, choose first no-unrelated-tile overlap, deterministic first-candidate fallback; all cable/C14 semantics and restrictions unchanged**
router-authority: **CLOSED DURING THIS FINITE CABLE-LABEL REPAIR — no third new executable path, gate/oracle/product-layout/topology change, hidden label, weakened collision proof, or different correction is authorized**
...
phase3-status: **BASE `f0d0634868b5b89e8b58f6600c2dff8cb129552a` / EXISTING AUTHORIZED DIRTY SET PLUS TWO-FILE SELECTED CABLE-LABEL REPAIR OPEN — parser and candidate/Golden remain closed; no candidate until P01-P17 are green**

...
status: G1–G12 CLOSED / PHASE 2 ACCEPTED / PHASE 3 FOUNDER-PATH F01 VERIFIER RED / BOUNDED AMENDMENT READER REQUIRED / P14-B RED / R18 CLOSED
...
builder-authority: **CLOSED pending the same semantic Reader's YES / YES reread of [the bounded F01 projection amendment](evidence/golden-baseline/phase3/PHASE3-FOUNDER-PATH-CURRENT-PRODUCT-CORRECTION-20260830.md); after YES / YES, reopen only for `task-delegation-projection.ts`, its unit test, the two named gate readers, the existing bounded build receipt, and generated Atlas outputs if regeneration changes them; preserve candidate `295fa47d` F02-F07 product/UI bytes and all existing meanings; no other product source, schema, ontology, provider/model, credential, Bovada, R18, premium redesign, or acceptance weakening**
...
phase3-status: **P14-B RED — prior evidence proves packaged launch/session/transcript only, not production inference; exactly one fresh `hermes-production-inference` proof is open; no candidate until P01-P17 are green**

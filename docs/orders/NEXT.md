...
phase3-p14-continuation: **P14-A/B/C/D remain mandatory and independently runnable; any RED blocks candidate/Golden but not the other three proofs. On P14-A admission RED after stored live capture, authorize only aggregate no-refetch analysis bound to source Artifact hash/execution source with exact source delta and predicate funnel counts; no raw JSON, teams, competitors, odds, headers, or names; source reachability is not admission and no acceptance is reclassified**

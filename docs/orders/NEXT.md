...
status: G1–G12 CLOSED / PHASE 2 ACCEPTED / PHASE 3 FOUNDER ONTOLOGY READ REPAIR OPEN / R18 CLOSED
...
builder-authority: **OPEN TO ONE BUILDER ONLY — implement the exact three-read orchestrator addition plus accepted touching-link gateway correction; editable product/proof paths are only `collab-electron/src/main/ontology-role-tools.ts`, `collab-electron/src/main/ontology-gateway.ts`, `collab-electron/src/main/ontology-gateway.test.ts`, `qa/gates/windows-dock-ontology.ts`, directly caused Phase-3 evidence, and generated Atlas outputs if changed; no provider call, schema, Kernel, generated ontology, MCP bridge, prompt, UI, P14-B, package-config, dependency, R18, bet, or trade change**
router-authority: **OPEN TO FREEZE THIS AUTHORITY, ROUTE ONE BUILDER, RUN PROVIDER-FREE FOCUSED/PACKAGED PROOF, FREEZE AND INDEPENDENTLY VERIFY ONE IMMUTABLE CANDIDATE, THEN REPEAT THE EXACT ROUTER-OWNED REAL FOUNDER WALKTHROUGH. No provider prompt until provider-free acceptance is green. Golden/main/remotes/R18 remain closed until final Phase-3 acceptance**
...
phase3-status: **FOUNDER ONTOLOGY READ REPAIR OPEN — exact packaged Research Director / real provider path is proven reachable, but the first founder walkthrough RED because its focused role surface omitted the existing generated Task/lineage reads; one bounded repair is open and no provider retry is permitted before provider-free acceptance**

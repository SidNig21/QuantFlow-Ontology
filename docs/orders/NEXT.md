...
builder-authority: **OPEN TO ONE BUILDER ONLY — implement the exact three-read orchestrator addition and proof matrix in the accepted founder Ontology read amendment; editable product/proof paths are only `collab-electron/src/main/ontology-role-tools.ts`, `collab-electron/src/main/ontology-gateway.test.ts`, `qa/gates/windows-dock-ontology.ts`, directly caused Phase-3 evidence, and generated Atlas outputs if changed; no provider call, schema, Kernel, MCP bridge, prompt, UI, P14-B, package-config, dependency, R18, bet, or trade change**

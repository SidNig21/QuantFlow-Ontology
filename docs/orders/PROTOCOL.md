...
**Atlas change-control rule:** before any IPC, SQL, or product edit, both Builder and Verifier read [`qf-atlas/ATLAS.md`](../../qf-atlas/ATLAS.md), run `bun qf-atlas/generate.mjs --check`, then `bun qf-atlas/ratchet.mjs`, and use that snapshot to understand the affected files and blast radius. After the focused product gates, regenerate with `bun qf-atlas/generate.mjs`, then run `bun qf-atlas/generate.mjs --check` and `bun qf-atlas/ratchet.mjs`, inspect and report `bun qf-atlas/generate.mjs --diff <pre-change-ref>`, and retain the unedited outputs. Baseline edits require explicit founder authority; Atlas never authorizes semantic changes or deletion. This normal-loop sequence stays under 60 seconds; `bun qf-atlas/falsify.mjs` is independent acceptance evidence, not a per-rung gate.

...
**Both doors get rotated, not just the builder's (added 2026-07-25).** The same passing commit must also confirm `docs/orders/VERIFYING.md` still names every authority document a verifier needs. This exists because it failed exactly once and silently: `VERIFYING.md` sat untouched from 2026-07-18 while `AGENTS.md`, `DOCTRINE.md` and `SCOPES.md` were created around it, so for a week **the seat with more authority entered through the worse map** — a verifier following it literally would have met a known-defective, already-routed type description with nothing telling them it was owned by a later rung, and failed the order or fixed it out of scope.

# WO-g1 â€” GLACIER foundation: tokens + terminal palette

status: AUTHORIZED â€” founder 2026-08-03 (GLACIER Scope B with A checkpoint)
branch: `wo-g1-glacier-foundation`
program: GLACIER full visual swap Â· order 1 of 5
ladder: **non-ladder.** Does not touch R0â€“R9, does not advance R9.
...
Spec: [`design/glacier/README.md`](../../design/glacier/README.md) Â·
[`tile-spec.html`](../../design/glacier/tile-spec.html) Â·
...
exist. The terminal palette rides along because it is the same concern â€” colour
authority â€” and it is most of every tile's pixels.
...
`brightBlack` matters most â€” agents use it for every dim line, box rule, and hint.

## Deliverables

1. **Append** `design/glacier/qf-tokens.additions.css` into
   `collab-electron/src/windows/shared/qf-tokens.css`. Append only â€” do not modify
...
2. **Replace** the body of `collab-electron/packages/components/src/Terminal/theme.ts`
   ...
   three exports (`darkTheme`, `lightTheme`, `getTheme`) â€” `Terminal/index.ts:2`
...
Tile chrome, dock, file rail, canvas, cables. Those are WO-g2â€¦g5.
...
4. **No visual regression.** Screenshot the shell, nav, viewer, and settings windows.
   *Fails if:* any changed â€” this order adds tokens, it does not consume them.
5. **Terminal receipt.** Spawn one `hermes-worker`, screenshot before/after into
   ...
   reaching xterm) or the tile is blank/error â€” the shots in `evidence/wo-006d/`
   are error states and are **not** a valid baseline.

## Notes

- `getTheme()` is called at `TerminalTab.tsx:44` and `:384`. Both must keep working.
- `background` goes from `rgba(8,8,8,0)` to opaque `#07090c`. **Intentional** â€” the

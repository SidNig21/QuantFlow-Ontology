...
status: done — independently verified PASS and merged 2026-07-28
assignee: builder
...
## Verification record — PASS (2026-07-28)

A separate Cursor Composer 2.5 verifier started from a clean worktree at exact commit `6c5be51` and
read the authority chain before inspecting the builder report. It independently re-derived zero
private Kernel aliases, 2 `qf-kernel` exports (`./portable` present), and 8 schema export-map entries
(`.` plus 7 string subpaths).

The verifier ran `bun qa/verify-release.ts` cold. The command exited `0` after 332 seconds: frozen
install; 258 Electron unit tests; production build with 177 main modules, 2 preload modules, and
10,031 renderer modules; then every QA gate, including `schema-bundle-aliases` and
`release-verifier`. This proves the shipped build completed before the green QA board.

All six required baits were re-run through their real checkers. The three package-coupling mutants
(`alias`, `exclude`, `manifest`) each exited `1` naming the planted defect, then restored green. The
three release-chain mutants (`stage`, `workflow`, `handbook`) each exited `1`; the stage mutant was
caught independently by both the declared contract and the real runner's fail-fast behavior, then
restored green. The worktree was clean before and after.

Seam inspection found no false-green path in scope. The independent stage oracle prevents a live
stage deletion from updating its own expected answer; an injected build exit `23` propagates and
prevents QA from running; CI has exactly one run command, `bun qa/verify-release.ts`. The verifier
accepted two authority carry-forwards rather than silently editing them: `PROTOCOL.md` still names
the legacy QA-only verifier at lines 75 and 94 and requires explicit founder authority to change;
the prior `NEXT.md` still described the superseded manifest-derived-alias design and is corrected by
this atomic rotation.

Ruling: **PASS**. Builder evidence is in
[`evidence/wo-ci1/REWORK-ROUND-1.md`](evidence/wo-ci1/REWORK-ROUND-1.md). Debt #31 is closed and the
ladder returns to WO-K3. No claim is made that `bun qa/run.ts --all` alone proves a shipped build.

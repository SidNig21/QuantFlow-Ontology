...
|---|---|---|---|
| **L0 Kernel** | SQLite (`bun:sqlite` in tests/gates; `node:sqlite` via the WO-006b driver seam inside Electron) + **Zod** as the single schema source | `defineObject/Link/Action` helpers → generators emitting SQL migrations, MCP tools, `ONTOLOGY.md` | Every tile's contents. The Artifact tile shows that artifact *because the Kernel says so* — not because React holds it |
...
**Two identity layers, not three — narrowed 2026-07-18 after adversarial review.** The measured claim is: the ACP guest mints the ID and AgentOS adopts it, ACP carries it in-band on every `sessionUpdate`, and cancel is routed by that ID. **`ToolLoopAgent` owns no session identity** — the AI SDK has no such concept (`ai@7.0.31`), so the loop runs *inside* the session's scope rather than carrying it. WO-004's original three-layer assertion echoed the ACP ID rather than measuring a third value; that assertion was corrected by WO-004a (merged 2026-07-18). The invariant that matters — **no second session identity, no second server** — holds and is measured.
...
**Native TUI surface (founder 2026-07-20).** If a species ships a real interactive TUI (e.g. Hermes `hermes --tui`), dock Spawn must open that TUI in a **term tile** — not a QuantFlow-invented chat chrome. ACP host bridges remain substrate where useful; they are not a substitute for the agent’s own UI. Applies to all future native-TUI species (WO-008d).

**The guest is a species, not a marriage — open-host principle (recorded 2026-07-19).** The ACP stdio seam proven by WO-004/004a is the deliberate swap point: any program that speaks ACP can be packed as AgentOS software and driven as a session guest — this is the documented AgentOS custom-agent path (single-process, like OpenCode). `ToolLoopAgent` is the v0.1 species. **Hermes** is the founder’s primary interactive agent — host process + native TUI (WO-008d); `hermes acp` remains available substrate. Three rules keep any guest from becoming Eve 2:
- **The Kernel stays sole writer for domain truth** — Hypothesis, Artifact, Evaluation, and `agent_session` identity. A guest's internal memory (e.g. Hermes' own skills/recall SQLite) is *agent-private working state*, never a QuantFlow truth store. Law E's gates already enforce this mechanically.
- **New tools the founder finds plug in at the edges** — as MCP servers, AgentOS software packages, or Kernel-backed generated tools under deny-by-default permissions. Never by forking the host, and never by hard-wiring one harness into the canvas. The host is built to stay open; species choice is cheap, host rewrites are not.
- **The dock (L4) is the spawn surface for species** — the founder's access point for launching guests onto the canvas. It is a Kernel projection like every tile (Laws A–C): the predecessor's dock is reference-only, and the rebuilt dock stores no truth of its own.

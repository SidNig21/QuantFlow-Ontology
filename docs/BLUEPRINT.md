...
**The guest is a species, not a marriage — open-host principle (recorded 2026-07-19).** The ACP stdio seam proven by WO-004/004a is the deliberate swap point: any program that speaks ACP can be packed as AgentOS software and driven as a session guest — this is the documented AgentOS custom-agent path (single-process, like OpenCode). `ToolLoopAgent` is the v0.1 species. **Hermes (`hermes acp`) is a recorded candidate second species** for the v0.5 agent-contracts work — the founder's Eve alternative. Three rules keep any guest from becoming Eve 2:

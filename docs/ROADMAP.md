...
| 21 | **`docs/ONTOLOGY_SCHEMA.md` describes a schema that no longer exists.** Measured at WO-102 verification: `event` ×12, `market` ×8, `odds_series` ×2, zero mentions of `market_event` / `instrument` / `venue` — and it already lacked WO-101's `mission` / `policy` / `environment`, so the class predates the rename. `doc-action-surface` cannot catch it: that gate covers the *action* list only, and actions kept their names — debt #0's fix covered one surface and the same drift reappeared one surface over. Structural point: the hand-written schema doc duplicates the generated, gate-checked `golden/ONTOLOGY.md` — one is maintained by machine, the other by nobody, and a stale authority doc reads exactly like a current one (`PROTOCOL.md` rot rule). The fixing order chooses: regenerate-and-gate the object surface too, or formally demote `ONTOLOGY_SCHEMA.md` to design-prose-only in `DOC_AUTHORITY_MAP.md` and point readers at `golden/ONTOLOGY.md` for the surface. Edit-by-order-only either way | **Trigger:** the next order touching the schema doc surface — WO-103 at the latest, since it changes the surface again |

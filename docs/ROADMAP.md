...
| 6 | **Zod↔Kernel `content_hash` contract disagreement — trigger FIRED 2026-07-26, routed.** `publish_artifact`'s schema requires `content_hash`; the Kernel computes it and only *verifies* when supplied (`create.ts:120-125`), and has always also accepted undeclared `bytes`/`path` (`create.ts:30-36`). Measured when WO-105's GATE 1 first enforced the declared shape: kernel suite 28 → 19 pass / 9 fail and `agent-path` PASS → FAIL, because the repo's own gate calls it with `path` and no `content_hash` (`qa/gates/agent-path/run.ts:200`). This entry's written trigger was "before MCP exposure" and GATE 1 is exactly that moment; WO-105 as first written did not reference it, and its pre-build read checked the order's citations but not its omissions — **a measured-claims table can be right in every row it has and wrong in the row it lacks** | **WO-105 deliverable 0** (correct the declaration to the real contract; a false declaration is doctrine A5 in the direction nobody reads it) |
